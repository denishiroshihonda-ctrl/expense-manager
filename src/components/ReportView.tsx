    ...
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-2 mb-4">
        {(['restaurant','transport','hotel','flight'] as Category[]).map((cat, i) => (
          <div key={cat} className="rounded-lg p-2 lg:p-3" style={{ background: 'var(--surf)', border: '1px solid var(--brd)' }}>
            <div className="text-[8px] lg:text-[9px] font-semibold uppercase tracking-wider mb-1" style={{ color: 'var(--tx3)' }}>{CAT[cat].icon} {CAT[cat].label}</div>
            <div className="text-xs lg:text-sm font-semibold" style={{ fontFamily: 'monospace', color: 'var(--tx)' }}>{fmt((totals as any)[cat])}</div>
            <div className="text-[9px] lg:text-[10px] mt-0.5" style={{ color: 'var(--tx3)' }}>{(counts as any)[cat]} itens</div>
          </div>
        ))}
        <div className="col-span-2 lg:col-span-1 rounded-lg p-2 lg:p-3" style={{ background: 'rgba(59,130,246,.1)', border: '1px solid #3b82f6' }}>
          <div className="text-[8px] lg:text-[9px] font-semibold uppercase tracking-wider mb-1" style={{ color: '#93c5fd' }}>💰 Total</div>
          <div className="text-xs lg:text-sm font-semibold" style={{ fontFamily: 'monospace', color: '#3b82f6' }}>{fmt(grand)}</div>
          <div className="text-[9px] lg:text-[10px] mt-0.5" style={{ color: '#3b82f6' }}>{report.expenses.length} documentos</div>
        </div>
      </div>
      ...
      <div
        ref={dropRef}
        className="rounded-xl p-4 lg:p-6 text-center cursor-pointer mb-4 transition-all"
      ...
      >
        ...
        <div className="text-[10px] lg:text-[11px] mb-3" style={{ color: 'var(--tx3)' }}>Notas fiscais, recibos, passagens</div>
        <button className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-xs font-medium text-white" style={{ background: '#3b82f6' }} onClick={e => { e.stopPropagation(); fileInputRef.current?.click(); }}>
          + Selecionar arquivos
        </button>
        <div className="hidden lg:flex gap-2 justify-center flex-wrap mt-3">
          ...
          <div className="flex gap-1.5 mb-2 flex-wrap">
            {CATS.map(c => (
              <button key={c.key} onClick={() => onFilterChange(c.key)}
                className={`px-2 lg:px-2.5 py-1 rounded text-[9px] lg:text-[10px] font-medium transition-all ${filter === c.key ? 'bg-blue-900/30 border-blue-700/50 text-blue-400' : 'text-slate-400'}`}
                style={{ border: `1px solid ${filter === c.key ? '#1d4ed8' : 'var(--brd)'}`, background: filter === c.key ? 'rgba(59,130,246,.15)' : 'var(--surf)' }}>
                {c.label}
              </button>
            ))}
          </div>
          <div className="rounded-xl overflow-x-auto" style={{ border: '1px solid var(--brd)' }}>
            <table className="w-full border-collapse min-w-[600px]">
              <thead>
                <tr style={{ background: 'var(--surf2)' }}>
                  {['Arquivo','Categoria','Estabelecimento','Data','Valor','Conf.',''].map(h => (
                    <th key={h} className="px-2 lg:px-3 py-2 text-left text-[8px] lg:text-[9px] font-semibold uppercase tracking-wider whitespace-nowrap" style={{ color: 'var(--tx3)', borderBottom: '1px solid var(--brd)' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {expenses.length === 0 ? (
                  ...
                  return (
                    <tr key={e.id} style={{ borderBottom: '1px solid var(--brd)' }}>
                      <td className="px-2 lg:px-3 py-2 text-[10px] lg:text-[11px] max-w-[100px] lg:max-w-[120px] truncate" style={{ color: 'var(--tx)' }}>{e.filename}</td>
                      <td className="px-2 lg:px-3 py-2"><span className="inline-flex items-center gap-1 px-1.5 lg:px-2 py-0.5 rounded text-[9px] lg:text-[10px] font-medium" style={{ color: cat.color, background: cat.bg, border: `1px solid ${cat.border}` }}>{cat.icon} {cat.label}</span></td>
                      <td className="px-2 lg:px-3 py-2 text-[10px] lg:text-[11px]" style={{ color: 'var(--tx)' }}>{e.establishment}</td>
                      <td className="px-2 lg:px-3 py-2 text-[10px] lg:text-[11px]" style={{ color: 'var(--tx3)', fontFamily: 'monospace' }}>{e.date}</td>
                      <td className="px-2 lg:px-3 py-2 text-[10px] lg:text-[11px]" style={{ fontFamily: 'monospace', color: 'var(--tx)' }}>{e.value != null ? fmt(e.value) : <span style={{ color: 'var(--tx3)' }}>—</span>}</td>
                      <td className="px-2 lg:px-3 py-2">
                        <div className="flex items-center gap-1">
                          <div className="w-8 lg:w-10 h-1.5 rounded-full overflow-hidden" style={{ background: 'var(--brd)' }}>
                            <div className="h-full rounded-full" style={{ width: `${e.confidence}%`, background: '#059669' }} />
                          </div>
                          <span className="text-[8px] lg:text-[9px] font-medium" style={{ color: '#047857' }}>{e.confidence}%</span>
                        </div>
                      </td>
                      <td className="px-2 lg:px-3 py-2">
                        <div className="flex gap-1">
                          <button onClick={() => onEditExpense(e.id)} className="w-5 lg:w-6 h-5 lg:h-6 rounded flex items-center justify-center text-[9px] lg:text-[10px] hover:bg-blue-900/40 hover:text-blue-400 transition-colors" style={{ border: '1px solid var(--brd)', color: 'var(--tx3)' }}>✏</button>
                          <button onClick={() => onRemoveExpense(e.id)} className="w-5 lg:w-6 h-5 lg:h-6 rounded flex items-center justify-center text-[9px] lg:text-[10px] hover:bg-red-900/40 hover:text-red-400 transition-colors" style={{ border: '1px solid var(--brd)', color: 'var(--tx3)' }}>✕</button>
                        </div>
                      </td>
                    </tr>
                  );
